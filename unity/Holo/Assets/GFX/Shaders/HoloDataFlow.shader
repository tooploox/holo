	...
	Properties
	{
		...
		_TriplanarMappingBlendSharpness("Blend Sharpness", Range(1.0, 16.0)) = 4.0
			...
			_EnvironmentColorZ("Environment Color Z (RGB)", Color) = (0.0, 0.0, 1.0, 1.0)
				...
				_ScaleFactor("Scale Factor", Range(.0,5.0)) = 1.0
	}
					...
					void geom(triangle v2g IN[3], inout TriangleStream<g2f> tristream)
					{
					 ...
					 not based on Mrtk2 standard shader.
						...
						*/
						...
						//float4 move0 = float4(0.01, 0.0, 0.0, 0.0) * _WeightFactor;
						//float4 move1 = float4(0.0, 0.01, 0.0, 0.0) * _WeightFactor;
						float minLength = 0.2;
						float x = 1;
						float y = 1;
						...
						float4 move0 = float4(x * ratio, y * ratio, z * ratio, 0.0) * 0.1 * length(IN[0].normal);
						if (length(IN[0].normal) < minLength)
							move0 = float4(x * ratio, y * ratio, z * ratio, 0.0) * 0.1 * minLength;
						...
						float ratio1 = 1 / sum1;
						float4 move1 = float4(x1 * ratio1, y1 * ratio1, z1 * ratio1, 0.0) * 0.1 * length(IN[0].normal);
						if (length(IN[0].normal) < minLength)
							move1 = float4(x1 * ratio1, y1 * ratio1, z1 * ratio1, 0.0) * 0.1 * minLength;

						float4 move2 = float4(IN[0].normal * _ScaleFactor, 1);
						if (length(IN[0].normal) < minLength)
							move2 = float4(IN[0].normal / length(IN[0].normal) * minLength * _ScaleFactor, 1);
						//-----
